...
export const createStudio = (formData) =>
  API.post('/studio', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
export const updateStudio = (id, formData) =>
  API.put(`/studio/${id}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
export const deleteStudio = (id) => API.delete(`/studio/${id}`);

// Availability
export const getAvailableSlots = (studioId) => API.get(`/availability/${studioId}`);
...
export const getReviewsByStudio = (studioId) => API.get(`/review/studio/${studioId}`);
