...
import { Mic, Calendar, PlusCircle, TrendingUp, Clock, CheckCircle, AlertTriangle, Users, DollarSign, Camera, Star } from 'lucide-react';
...
const OwnerDashboard = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const myStudios = studiosResponse.data.filter(studio => {
          const authorId = typeof studio.author === 'object' ? studio.author._id : studio.author;
  ...
  const upcomingBookings = bookings.filter(booking => {
    ...
    return isAfter(bookingDate, now) || format(bookingDate, 'yyyy-MM-dd') === format(now, 'yyyy-MM-dd');
  });
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-2">
                  Studio Owner Dashboard 🎙️
                </h1>
                <p className="text-gray-600 text-lg">Manage your podcast studios and grow your business</p>
              </div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300 group">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-gray-500 text-sm font-medium">Active Studios</p>
                      <p className="text-3xl font-bold text-gray-900 mt-1">{approvedStudios.length}</p>
                    </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300 group">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-gray-900 mt-1">{pendingStudios.length}</p>
                    </div>
                ...
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                  ...
                  <div className="space-y-4">
                    ...
                    >
                      ...
                      <div>
                        <h3 className="font-semibold text-gray-900">Add New Studio</h3>
                        <p className="text-gray-600 text-sm">List a new podcast studio for booking</p>
                      </div>
                    </Link>
                    ...
                    >
                      ...
                      <div>
                        <h3 className="font-semibold text-gray-900">Manage Studios</h3>
                        <p className="text-gray-600 text-sm">Edit and manage your existing studios</p>
                      </div>
                    </Link>
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-gray-600 text-sm">Check bookings received for your studios</p>
                      </div>
                    </Link>
                  </div>
                </div>
                    ...
                    {approvedStudios.length > 0 ? (
                        ...
                        <div className="flex items-center text-green-800">
                          ...
                          <div>
                            <span className="font-semibold">
                              {approvedStudios.length} active {approvedStudios.length === 1 ? 'studio' : 'studios'}
                            </span>
                            <p className="text-sm text-green-600 mt-1">Ready to receive bookings</p>
                          </div>
                        </div>
                      </div>
                    ) : (
                      <div className="p-4 bg-gray-50 border border-gray-200 rounded-xl">
                        <div className="flex items-center text-gray-800">
                          ...
                          <div>
                            <span className="font-semibold">No active studios yet</span>
                            <p className="text-sm text-gray-600 mt-1">Add your first studio to start earning</p>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {pendingStudios.length > 0 && (
                      <div className="p-4 bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-xl">
                        <div className="flex items-center text-yellow-800">
                          ...
                          <div>
                            <span className="font-semibold">
                              {pendingStudios.length} {pendingStudios.length === 1 ? 'studio' : 'studios'} pending approval
                            </span>
                            <p className="text-sm text-yellow-600 mt-1">Waiting for admin review</p>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="p-4 bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-xl">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center text-purple-800">
                          ...
                          <div>
                            <span className="font-semibold">This Month's Revenue</span>
                            <p className="text-sm text-purple-600 mt-1">₹{monthlyRevenue.toLocaleString()}</p>
                          </div>
                        </div>
              ...
              <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 mb-8">
                ...
                {bookings.length > 0 ? (
                    ...
                    {bookings.slice(0, 5).map((booking) => (
                      <div key={booking._id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
                        <div className="flex items-center">
                          ...
                          <div>
                            <h4 className="font-semibold text-gray-900">{booking.studio?.name}</h4>
                        ...
                        <div className="text-right">
                          <div className="text-lg font-bold text-indigo-600">₹{booking.totalPrice}</div>
                          <div className={`text-xs px-2 py-1 rounded-full mt-1 ${
                            isAfter(parseISO(booking.date), now) 
                              ? 'bg-green-100 text-green-800' 
                              : 'bg-gray-100 text-gray-800'
                          }`}>
                            {isAfter(parseISO(booking.date), now) ? 'Upcoming' : 'Completed'}
                          </div>
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-600 mb-4">Once you have approved studios, bookings will appear here</p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-8 rounded-2xl text-white">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-white/10 backdrop-blur-sm p-6 rounded-xl">
                    ...
                    <p className="text-indigo-100 text-sm">Upload professional photos of your studio to attract more bookings and showcase your equipment.</p>
                  </div>
                  ...
                  <div className="bg-white/10 backdrop-blur-sm p-6 rounded-xl">
                    ...
                    <h3 className="font-semibold mb-2">Keep Availability Updated</h3>
                    <p className="text-indigo-100 text-sm">Regularly update your availability calendar to avoid scheduling conflicts and maximize bookings.</p>
                  </div>
                  ...
                  <div className="bg-white/10 backdrop-blur-sm p-6 rounded-xl">
                    ...
                    <p className="text-indigo-100 text-sm">Provide exceptional service to earn positive reviews and build a strong reputation in the community.</p>
                  </div>
                </div>
              </div>
              ...
              {pendingStudios.length > 0 && (
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 mt-8">
                  <h2 className="text-2xl font-bold text-gray-900 mb-6">Studios Pending Approval</h2>
                  <div className="space-y-4">
                    {pendingStudios.map(studio => (
                      <div key={studio._id} className="flex items-center justify-between p-4 border border-yellow-200 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl">
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">Submitted on {format(new Date(studio.createdAt), 'MMMM d, yyyy')}</p>
                          </div>
                        </div>
