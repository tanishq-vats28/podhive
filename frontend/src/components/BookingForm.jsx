...
const BookingForm = ({ studio, selectedSlot, onBookingComplete }) => {
  ...
  const { date, hours, packageKey, package: selectedPackage, basePrice } = selectedSlot;
  ...
  const calculateTotalPrice = () => {
    const addonTotal = selectedAddons.reduce((total, selectedAddon) => {
      const studioAddon = studio.addons?.find(addon => addon.key === selectedAddon.key);
      return total + (studioAddon ? studioAddon.price * selectedAddon.quantity : 0);
    }, 0);
    ...
    <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium text-gray-900">
              {hours.map(h => formatHour(h)).join(', ')} ({hours.length} hour{hours.length > 1 ? 's' : ''})
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600">Package:</span>
            <span className="font-medium text-gray-900">{selectedPackage.name || packageKey}</span>
          </div>
        ...
        <div className="mb-6">
          ...
          <div className="space-y-3">
            {studio.addons.map((addon) => (
              <div key={addon.key} className="p-3 border border-gray-200 rounded-lg">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex-1">
                    <h5 className="font-medium text-gray-900">{addon.name}</h5>
                    <p className="text-sm text-gray-600">{addon.description}</p>
                    <p className="text-sm font-semibold text-indigo-600">₹{addon.price} each</p>
                  </div>
                  <div className="flex items-center space-x-2 ml-4">
                    <button
                      type="button"
                      onClick={() => handleAddonQuantityChange(addon.key, Math.max(0, getAddonQuantity(addon.key) - 1))}
                      className="w-8 h-8 rounded-full bg-gray-200 hover:bg-gray-300 flex items-center justify-center transition-colors"
                      disabled={getAddonQuantity(addon.key) === 0}
                    >
                      <Minus className="h-4 w-4" />
                    </button>
                    <span className="w-8 text-center font-medium">{getAddonQuantity(addon.key)}</span>
                    <button
                      type="button"
                      onClick={() => handleAddonQuantityChange(addon.key, Math.min(addon.maxQuantity || 10, getAddonQuantity(addon.key) + 1))}
                      ...
                      disabled={getAddonQuantity(addon.key) >= (addon.maxQuantity || 10)}
                    >
                ...
                {addon.maxQuantity && (
                  <p className="text-xs text-gray-500">Max quantity: {addon.maxQuantity}</p>
                )}
              </div>
            ))}
          </div>
      ...
      <div className="mb-6 p-4 bg-gray-50 rounded-lg">
        <div className="flex justify-between items-center">
          ...
          <span className="text-2xl font-bold text-indigo-600">₹{calculateTotalPrice()}</span>
        </div>
              ...
              {selectedAddons.map(selectedAddon => {
                ...
                return studioAddon ? (
                  <div key={selectedAddon.key} className="flex justify-between">
                    <span>{studioAddon.name} × {selectedAddon.quantity}:</span>
